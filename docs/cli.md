...
| `npm run verify:retrieval-gates` | **五道检索闸逐一失效，每次金标都必须变红**。README 与 `docs/retrieval.md` 都声称「每道闸都有专属用例」，而**此前没有任何门禁核它**——它曾经是假的（`log.md` 记着「一道闸被另一道遮住时，关掉它金标不会红」）。实测结果：**五道都会让金标变红，但只有四道有「独有」金标**；接缝识别那道的两条红项已登记为已知局限，所以它没有自己的。该结论已写进 `docs/retrieval.md`（那里原先给它列了一条**不存在的**专属用例） |
