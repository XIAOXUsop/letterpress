...
| `npm run check:single-source` | **版本号只有一处真值**。`CONTENT_MANIFEST_VERSION` 的真值在 `src/lib/content-manifest.ts`；任何地方再写一遍 `MANIFEST_VERSION = 2` 都会红（注释里的不算）。这条来自一次**真故障**：提交把生产端升到 v2 时同步器与它的测试固件都没跟上，于是**同步器对着本站自己的清单必然报错，而 473 条测试全绿**——因为测试固件也写着旧值，**测的是一个已不存在的格式** |
