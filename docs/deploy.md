...
**没配的话这个 job 会以 `skipped` 出现（灰色），不是绿。** 这一点是刻意的：
把一个没跑过的检查画成绿的，是这类检查最常见的失效方式。
实测确认过（2026-09-19，run 35417922989）：4 个 job 里它是唯一一个 `skipped`，其余三个绿。

> 这里原先写的是「没配的话这个 job **不会出现**」——**那是错的**，它会出现在列表里、
> 只是状态是 `skipped`。同一句话在 `.github/workflows/ci.yml` 里已经改对了，
> 但当时只改了那一处——**同一个事实有好几个表面，改一处不等于改完了**。
>
> ⚠️ 但也别把 `skipped` 当成"通过"来读：它同样不告诉你任何关于内容协商的事。
