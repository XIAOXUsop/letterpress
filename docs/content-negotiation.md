...
/markdown-for-agents/            6750       2852    57.7%
/cjk-web-typography/             8732       3919    55.1%
/wiki/content-negotiation/       4390       1106    74.8%
```

> **2026-09-24 前两页的数字又涨了、节省率又掉了**（上一版为 6214/61.5% 与 7614/61.1%）。
> 原因是迭代 D 的内容修订给这两篇补了规范原文、`ic` 单位的定义与字重匹配算法，
> 而**这些新增文字在 HTML 与 markdown 里都有**——分母分子同时变大，节省率因此下降。
> **机制与 09-22 那次完全一样**，这也说明它是这套内容维护的常态成本，不是回退。
> 内容协商省的是**结构与样式**（导航、页脚、`<div>` 嵌套），正文本身在两种表示里都在。
