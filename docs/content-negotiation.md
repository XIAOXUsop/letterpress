...
>
> 2026-09-24 用真实词表 `o200k_base` 在**同一批文本**上复算
> （估算取 `estimateTokens` 自身，真实取该编码器对同一字符串的结果），
> **估算稳定偏低 9%–14%**：
>
> | 页面 | HTML 估算 / 真实 | MD 估算 / 真实 | 节省 估算 / 真实 |
> |---|---:|---:|---:|
> | /markdown-for-agents/ | 6,750 / 7,530（−10.4%） | 2,852 / 2,965（−3.8%） | 57.7% / **60.6%** |
> | /cjk-web-typography/ | 8,743 / 10,109（−13.5%） | 3,927 / 4,256（−7.7%） | 55.1% / **57.9%** |
> | /wiki/content-negotiation/ | 4,390 / 4,787（−8.3%） | 1,106 / 1,103（+0.3%） | 74.8% / **77.0%** |
>
> **偏差两边都有，且不固定**：HTML 侧稳定低估 8–14%，markdown 侧从 −7.7% 到 +0.3%。
> 所以估算值只能当量级，不能当用量。
> 低估来自中文——真实词表会把常见双字词合成一个 token，而按「1 字 1 token」数
> 会把它们各算一个。**结论方向不受影响**（省得比这里写的更多），但把估算值
> 说成「实测」「真实数字」是不对的，这一节原先就是这么写的。
>
> ⚠️ **这张表量的是当次构建的产物，内容一改就过期。** 本轮它漂过一次：
> `README.md` 仍写着上一版的 61.5% / 61.1%，是 `npm run verify` 的契约断言
> 发现的——**这类数字靠手抄必然漂，且漂了通常没人提醒**。
> 重算时用 `estimateTokens` 跑产物，再用 `o200k_base` 编码同一文本，别手抄。
...
/cjk-web-typography/             8743       3927    55.1%
