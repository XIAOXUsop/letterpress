...
> **2026-09-24 恢复、2026-09-25 按最新产物复测**：那张表曾在一次「移除与站点使用无关的设计与过程记录」的
> 改动里被删了——**而它不是过程记录，是一组可复现的实测**（用 `tiktoken` 的
> `o200k_base` 在**同一批文本**上复算，口径与 `estimateTokens` 可比）。
>
> 当场重跑，结论与当初一致——**估算在 HTML 侧稳定偏低 8–14%**，
> 而 markdown 侧从 −7.7% 到 +0.3%：
>
> | 页面 | HTML 估算 / 真实 | MD 估算 / 真实 | 节省 估算 / 真实 |
> |---|---:|---:|---:|
> | /markdown-for-agents/ | 6,779 / 7,556（−10.3%） | 2,852 / 2,965（−3.8%） | 57.9% / **60.8%** |
> | /cjk-web-typography/ | 8,762 / 10,121（−13.4%） | 3,919 / 4,246（−7.7%） | 55.3% / **58.0%** |
> | /wiki/content-negotiation/ | 4,419 / 4,813（−8.2%） | 1,106 / 1,103（+0.3%） | 75.0% / **77.1%** |
>
> **方向结论不受影响**（省得比这里写的更多），但**这两组数不能混着用**：
> 估算是给 agent 的量级提示，真实词表那组才是与模型用量可比的。
>
> ⚠️ **这张表量的是当次构建的产物，内容一改就过期**——
> 所以它不在门禁里（门禁只核上面那张估算表，那是每次构建都会重打的）。

`npm run verify` 会打印估算值：

```
页面                         HTML token   MD token       节省
/markdown-for-agents/            6779       2852    57.9%
/cjk-web-typography/             8762       3919    55.3%
/wiki/content-negotiation/       4419       1106    75.0%
```
...
本项目生成它是因为零成本，**但不把它当卖点**。本站实际可用的是显式 `.md`
链接与内容清单；内容协商需要部署在支持边缘改写的平台上。
