 ...
 *
 * 判据是「面向用户的 CLI 不许用未归类的 `exit(1)`」，外加**码表自身的完整性**。
 * 这类检查很容易**看着在管、实际漏一大片**，所以这里逐条注入，每一种都必须真的变红：
 *
 *   ① 写回字面 `process.exit(1)`         → 「未归类」那条必须红
 *   ② 写一个不存在的常量名              → 「认不出来的写法」那条必须红
 *   ③ 写一个存在但未在码表里登记的数字   → 「没有登记」那条必须红
 *   ④ 码表少一个码（制造不连续）        → 「码表不连续」那条必须红
 *   ⑤ 码表缺 1                          → 「码表里没有 1」那条必须红
 *
 * ② 与 ③ 是**两种完全不同的失败**：
 ...
 * 而 ③ 恰恰是最该被抓住的：**它不会自己暴露**。
 *
 * ④⑤ 与 ①②③ 形状不同：它们要改的是**码表自己**（`src/lib/cli/exit-codes.mjs`），
 * 而 ①②③ 改的是 CLI 的 `process.exit` 调用。**两类注入点不能互相覆盖**——
 * 此前只有后者，于是码表的四条判据一条都没被验过。
 *
...
const MUTATIONS = [
  /*
   * ④⑤ 验的是**码表自身**的四条判据（有 0 / 有 1 / 连续 / 不超过 10 个）。
   *
   * ⚠️ 2026-09-24 才补的，而它们此前**一条都没被负向验证覆盖**——
   * 上面三条都靠改 CLI 源码触发，**码表那四条得改 `exit-codes.mjs` 自己**，
   * 而没有人想到要改它。
   *
   * > 「变异的注入点随实现移动」这条约定（见 26 行的注释）在这里也要用：
   * > 注入点是**码表里的那一行**，不是任何 CLI 的 `process.exit`。
   *
   * 补之前先手工验过它们**真的会红**（删掉码 4 → 「3 之后是 5」，
   * 退出码 1）——**先确认判据有效，再把它纳入负向验证**，
   * 否则可能把一条恒真的判据当成「已覆盖」。
   */
  {
    name: '④ 码表少一个码（制造不连续）',
    file: 'src/lib/cli/exit-codes.mjs',
    replace: [
      /  \[EXIT_EMPTY_INPUT\]: '[^']*',\n/,
      '',
    ],
...
for (const m of MUTATIONS) {
   ...
   * `find` 可以是**字符串**或**正则**——
   * 码表那两条变异删的是整行（`  [EXIT_X]: '…',\n`），
   * 用字符串写会因为具体文案随迭代变而漂，用正则只锚定结构。
   */
  const matched =
    find instanceof RegExp ? find.test(original) : original.includes(find);
  if (!matched) {
    const what = find instanceof RegExp ? find.source : find;
    console.log(`  ✗ ${m.name}：锚点 "${what}" 在 ${m.file} 里找不到（源码已漂，变异没注入）`);
    allGood = false;
    continue;
  }
...
console.log(
  allGood
    ? `${MUTATIONS.length} 种破坏都真的报出来了——这道门禁是尺子，不是装饰。`
    : '有变异没报出来——门禁有洞。',
);
console.log();
