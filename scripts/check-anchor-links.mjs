 ...
 * ── 为什么单独一个门禁 ──────────────────────────────────────────────
 *
 * 2026-09-24 实测：`docs/features.md` 与 `docs/design-notes.md` 都链到
 * `compare.md#关于站内-js-这一行`，而那个标题的真锚点是
 * **`关于站内-js这一行`**（**少一个连字符**）——两处都点不到。
 *
 * 根因：标题是 `## 关于「站内 JS」这一行`，
 * 而中文引号被删后 `JS」这一行` 里的空格与引号组合让 slug 少一个连字符。
 * **这不是我推的，是抓 GitHub 线上真实 HTML 核实的：**
 * `curl https://github.com/XIAOXUsop/letterpress/blob/main/docs/compare.md`
 * 里那个标题的 id 是 `user-content-关于站内-js这一行`。
 *
