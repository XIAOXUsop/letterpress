...
| 站内 JS | 文章页 **0 个文件**；只有搜索页按需加载同源 Pagefind 1.5.2，实测运行时资源 **6 个 js**、未压缩共 **431 KB**（441271 bytes）；其中**访问者真正会加载的只有 3 个 UI 包、302 KB**（gzip 后 69 KB），其余是 `pagefind.js` / `worker` 等由 loader 按需取的部分（见 `docs/compare.md`）。复测日期 2026-09-24；此前写的「17 个文件、146 KB gzip」在当前环境**已无法复现**（pagefind 版本未变而产物结构对不上），故改为可复现的口径 |
