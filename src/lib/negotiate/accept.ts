 ...
 *
 * ⚠️ **但下面第 1 条的「平局靠前胜出」是本项目的选择，不是 RFC 的规定。**
 * 规范只按媒体范围的**具体程度**定优先级（更具体的覆盖更宽的），
 * 对 q 值相同的两个类型只说它们是 "equally preferred"，**没有给排序规则**。
 * 原先这里用「依据 RFC …」起头，很容易被读成那条平局规则也来自规范。
 *
 * ── 三件必须做对的事 ──────────────────────────────────────────────
 *
 * 1. **缺省 q 值是 1.0；并列时本项目让靠前的胜出（本地策略，规范未规定）。**
