...
describe('estimateTokens', () => {
  ...
  it('西文部分向上取整（5 个字符算 2 而不是 1）', () => {
    // ⚠️ **测试名以前写的是「宁可高估也不低估」——那是错的。**
    // 它与 `estimateTokens` 上方的注释矛盾，而那条注释是本轮（2026-09-24）
    // 刚改正过的：原注释声称「宁可偏保守（高估）」，**实测是反的**——
    // 用 `o200k_base` 对三个页面复算，这个估算**稳定低估 9%–14%**。
    //
    // 「向上取整」只对**西文那一半**成立（`ceil(other / 4)`），
    // 它管不到中文：中文按「1 字 1 token」算，而真实词表会把常见双字词
    // 合成一个 token——**低估恰恰都发生在中文那一侧**。
    //
    // 所以这条测试只钉住取整行为，**不声称整体是保守的**。
    // 整体偏低的实测数字记在 `docs/content-negotiation.md` 的对照表里。
